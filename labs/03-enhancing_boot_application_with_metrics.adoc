...
. Edit the following file: */cloud-native-spring/pom.xml* Add the _git-commit-id-plugin_ to your pom like so : 
+
...
The effect of all this configuration is that the _git-commit-id-plugin_ plugin adds Git branch and commit coordinates to the */actuator/info* endpoint.
...
*What Just Happened?*
+
By including the _git-commit-id-plugin_ plugin, details about git commit information will be included in the */actuator/info* endpoint. Git information is captured in a _git.properties_ file that is generated with the build. Review the following file: */cloud-native-spring/build/resources/main/git.properties*
...
---------------------------------------------------------------------
+
Note we're defining token delimited value-placeholders for each property.  In order to have these properties replaced, we'll need to add some further instructions to the _pom.xml_ file.
+
...
. Add the following directly underneath the _<plugins>_ block within *cloud-native-spring/pom.xml*
+
[source,xml]
---------------------------------------------------------------------
<plugin>
          ...
          <execution>
            ...
            <configuration>
              <target name="replaceLocalTokens">
                ...
                <replace file="src/main/resources/application.yml" token="@project.name@" value="${application.name}"/>
                <replace file="src/main/resources/application.yml" token="@project.description@" value="${application.description}"/>
                <replace file="src/main/resources/application.yml" token="@project.version@" value="${version}"/>
              </target>
            </configuration>
            <goals>
              <goal>run</goal>
...
*What Just Happened?*
+
We have mapped properties into the /actuator/info endpoint.
+
